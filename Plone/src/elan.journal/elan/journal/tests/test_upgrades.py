...
class UpgradeBaseTestCase(unittest.TestCase):
    ...
    profile_id = u'elan.journal:default'
...
class To1002TestCase(UpgradeBaseTestCase):
    ...
    def test_remove_workflow(self):
        title = u'Migrate journal workflow'
        step = self._get_upgrade_step_by_title(title)
        assert step is not None

        # simulate (partially) state on previous version
        wtool = api.portal.get_tool('portal_workflow')
        wtool.setChainForPortalTypes(('Journal',), ('journal_workflow',))
        assert wtool.getChainForPortalType('Journal') == ('journal_workflow',)

        # execute upgrade step and verify changes were applied
        self._do_upgrade(step)

        self.assertEqual(
            wtool.getChainForPortalType('Journal'),
            ('simple_publication_workflow',)
        )

    @unittest.skipIf(IS_PLONE_5, 'Not needed in Plone 5')
    def test_make_journal_linkable(self):
        title = u'Make Journal linkable on TinyMCE'
        step = self._get_upgrade_step_by_title(title)
        assert step is not None
        ...
        linkable.remove('Journal')
        tinymce.linkable = '\n'.join(linkable)
        assert 'Journal' not in tinymce.linkable.split('\n')
