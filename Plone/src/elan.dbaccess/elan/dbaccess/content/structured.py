...
from elan.dbaccess.content.registry import registerEntityConfig
from formalchemy.fields import HiddenFieldRenderer
from elan.dbaccess.content.forms import NoRenderer
...
class StructuredEntity(object):
    ...
    def fields(cls):
        ...
        cm = class_mapper(cls)
        if cm:
            fields = [f for f in cm.columns.keys() if not (f.find("_id") > 0 or f == "id")]
            #print cm.columns
...
#            g.insert(g[fields[0]], Field('check', type=types.String, value=lambda item: "<input type='checkbox' name='objsel:list' value=\"%s\"/>" % (str(item._sa_instance_state.key[1]))))
    ...
    def list_fs(cls, context):
        ...
        """
        g = cls.grid()
        g.append(Field('editlink', type=types.String, value=lambda item: "<a href=\"%s/struct_edit?typ=%s&pk=%s&bc=%s\">" % (context.getContextObj().absolute_url(), cls.typename(), str(item._sa_instance_state.key[1]), context.getView().parentView.breadcrumbs()) + _("Edit")+"</a>"))
    ...
    @classmethod
    def register(cls):
        registerEntityConfig(cls.typename(), cls, gen_fs=True, protect=True,
                     list_fs = cls.list_fs,
                     edit_fs = cls.edit_fs,
                     create_fs=cls.create_fs, 
                     sort_default='id', label=_(cls.__name__))
