# -*- coding: utf-8 -*-
#
# File: elancollaborationfolder.py
#
...
"""Definition of the ELANCollaborationFolder content type. See elancollaborationfolder.py for more
...
from elan.esd.utils import getAllowedELANDocumentTypes,\
    getAllowedELANDocumentTypesForGroup
...
class IELANCollaborationFolder(form.Schema, IELANFolder):
    """
    """
        
    allowedPartnerDocTypes = schema.List(
                        title=_(u'label_elancollaborationfolder_allowedpartnerdoctypes', default=u'Document types allowed for the guest group(s)'),
                        description=_(u'description_elancollaborationfolder_allowedpartnerdoctypes', default=u''),
                        required=True,
##code-section field_allowedPartnerDocTypes
                        value_type=schema.Choice(source="elan.esd.vocabularies.ELANGroupDocType"),
##/code-section field_allowedPartnerDocTypes                           
    )
...
class ELANCollaborationFolder(Container, ELANFolder):
    """
    """
    security = ClassSecurityInfo()
    
    implements(IELANCollaborationFolder)
...
##code-section methods
    def createActions(self):
        ...
        placefulWfName = 'elan-collaboration-folder'
    ...
    def customMenu(self, menu_items):
        ...
        if not "Reviewer" in user.get_roles(obj=self):
            return ELANFolder.customMenu(self, menu_items)
        else:
            print "Reviewer"
            dts = getAllowedELANDocumentTypesForGroup(self)
            filter = False
            ...
            for menu_item in menu_items:
                if menu_item.get('id') == 'ELANDocument':
                    for dt in dts:
                        print dt.id
             ...
             {'separator': None, 'id': dt.id, 'class': 'contenttype-%s' % dt.id}, 
                                        ...
                                        'description': '', 
                                        'title': dt.Title, 
                                        'action': '%s/++add++ELANDocument?form.widgets.docType:list=%s' % (self.absolute_url(), dt.id), 
                                        'selected': False, 
                                        'id': dt.id, 
                                        'icon': None})
                else:
...
##/code-section methods 
    ...
    def myELANCollaborationFolder(self):
        """
        """
        return self
