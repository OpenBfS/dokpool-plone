...
SITUATION_OVERVIEW = [
    {TYPE: 'SituationOverview', TITLE: 'Situation Overview', ID: 'sitview'}
]
MODULE_TYPES = [
...
MODTYPES = [
    {
        TYPE: 'SRModuleTypes',
        TITLE: 'SR Module Types',
        ID: 'mtypes',
        CHILDREN: MODULE_TYPES,
    }
]
SRCONFIG = [
...
def referenceTextBlocksAndCollections(self):
    ...
    mconfigs = [
        mc.getObject()
        for mc in queryForObjects(self, path=path, portal_type='SRModuleConfig')
    ]
    tbs = [
        tb.getObject()
        for tb in queryForObjects(self, path=path, portal_type='SRTextBlock')
    ]
    colls = [
        coll.getObject()
        for coll in queryForObjects(self, path=path, portal_type='SRCollection')
    ]
...
def createSituationReport(self):
    ...
    for mt in ModuleTypesVocabularyFactory(self, raw=True):
        ...
        if not hasattr(path, docid):
            path.invokeFactory(
                id=docid, type_name="SRModule", title=mt[1], description=""
            )
            d = path._getOb(docid)
            d.text = RichTextValue(safe_unicode(t))
            d.docType = mt[0]
