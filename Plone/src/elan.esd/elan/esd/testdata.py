...
def createTestDocuments(context, count):
    ...
    for i in range(1, count):
        ...
        path.invokeFactory(
            id=docid,
            type_name="DPDocument",
            title="Document %d %s" % (i, uname),
            description=d,
        )
        d = path._getOb(docid)
        d.docType = etype
...
def createGroupsAndUsers(context):
    ...
    title = context.Title()
    props = {
        'allowedDocTypes': alldtypes,
        'title': 'Group 1 (%s)' % title,
        'description': 'Test Group 1',
        'dp': context.UID(),
    }
    gtool.addGroup("%s_Group1" % prefix, properties=props)
    props = {
        'allowedDocTypes': somedtypes,
        'title': 'Group 2 (%s)' % title,
