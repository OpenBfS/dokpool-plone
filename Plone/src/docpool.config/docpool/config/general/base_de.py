...
def configUserFolders(self, fresh):
    ...
    if fresh:
        mtool.addMember('dpadmin', 'Docpool Administrator (global)', ['Site Administrator', 'Member'], [])
        dpadmin = mtool.getMemberById('dpadmin')
        dpadmin.setMemberProperties(
            {"fullname": 'Docpool Administrator'})
        ...
        dpmanager.setMemberProperties(
            {"fullname": 'Docpool Manager'})
